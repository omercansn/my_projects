...
class kanvas02 extends Canvas implements KeyListener
{
	...
	//private int FPS = 100;
	boolean isPressed = false, success = false, done = false;
	public kanvas02()
	{
	...
	public void level_1(Graphics p){
		
		if(success) {
			p.setFont(f1);
			p.drawString("YOU WON!", 400, 300);
			p.setFont(f2);
			p.drawString("Please press ENTER to proceed the next level.", 270, 400);
			//p.dispose();
		} else {
	...
	public void level_2(Graphics p){
	...
	public void level_3(Graphics p){
